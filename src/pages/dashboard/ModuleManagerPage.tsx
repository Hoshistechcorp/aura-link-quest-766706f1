...
import { Plus, Trash2, ImagePlus, Star, EyeOff, GripVertical, ExternalLink, Pencil } from "lucide-react";
...
const ItemRow = ({ item, config, onEdit, onDelete }: {
  item: ContentItem;
  config: ModuleConfig;
  onEdit: () => void;
  onDelete: () => void;
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {item.price && <span className="text-sm font-bold text-primary mr-2">${item.price}</span>}
          <button onClick={onEdit} className="p-2 rounded-lg hover:bg-primary/10 hover:text-primary transition-colors">
...
const ModuleManagerPage = () => {
  ...
  const handleSave = () => {
    if (!draft) return;
    if (isNew) {
      addItem(config.contentType, draft);
      toast({ title: `${config.itemLabel} added`, description: "Published to your microsite." });
    } else {
      updateItem(config.contentType, draft.id, draft);
      toast({ title: `${config.itemLabel} updated` });
    }
    setModalOpen(false);
    setDraft(null);
  };
  ...
  const handleDeleteFromModal = () => {
    if (!draft || isNew) { setModalOpen(false); setDraft(null); return; }
    deleteItem(config.contentType, draft.id);
    toast({ title: `${config.itemLabel} deleted` });
    setModalOpen(false);
    setDraft(null);
  };
      ...
      <div className="space-y-2 rounded-2xl bg-card border p-3 sm:p-4">
        {items.length === 0 ? (
          <div className="text-center py-12 text-sm text-muted-foreground">
            ...
            <button onClick={openAdd} className="text-primary text-xs font-medium hover:underline">
          ...
          items.map((item) => (
            <ItemRow
              ...
              onDelete={() => { deleteItem(config.contentType, item.id); toast({ title: `${config.itemLabel} deleted` }); }}
            />
          ))
        )}
      </div>

      <Dialog open={modalOpen} onOpenChange={(o) => { if (!o) { setModalOpen(false); setDraft(null); } }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{isNew ? `Add ${config.itemLabel}` : `Edit ${config.itemLabel}`}</DialogTitle>
            <DialogDescription>
              {isNew ? `Fill in the details to add a new ${config.itemLabel.toLowerCase()} to your microsite.` : `Update this ${config.itemLabel.toLowerCase()}.`}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid sm:grid-cols-2 gap-3 py-2">
              {config.fields.map((field) => (
                <div key={field.key} className={field.type === "image" || field.type === "textarea" || (!field.half && !field.third) ? "sm:col-span-2" : ""}>
                  <FieldInput field={field} item={draft} onUpdate={updateDraft} />
                </div>
              ))}
            </div>
          )}
          ...
          <DialogFooter className="gap-2">
            {!isNew && (
              <button onClick={handleDeleteFromModal} className="px-3 py-2 rounded-xl text-destructive hover:bg-destructive/10 text-sm font-medium mr-auto flex items-center gap-1.5">
                <Trash2 className="w-4 h-4" /> Delete
              </button>
            )}
            <button onClick={() => { setModalOpen(false); setDraft(null); }} className="px-4 py-2 rounded-xl bg-muted text-sm font-medium hover:bg-muted/80 transition-colors">
              Cancel
            </button>
            <button onClick={handleSave} className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">
              {isNew ? `Add ${config.itemLabel}` : "Save changes"}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
